...
[1., 2/3., 3/4., 9/10., 9/8., 6/5., 4/3., 3/2.] @=> float scale[]; // allowed notes. careful with the b7
[3/4., 1., 9/8., 6/5., 4/3.] @=> float init_seq[];
// [3/4., 5/6., 1., 9/8., 5/4., 3/2., 5/3.] @=> float init_seq[];  // Penta Min
1 => int REHEARSAL;
...
fun void add_seq(string filepath, int idx, float gain, float off, float deg) {
  ...
  seq_gran.init("drones/" + filepath, SEQ_TYPE);
...
fun void add_drone(string filepath, int idx, float gain, float off, float deg) {
  ...
  drone.init("drones/" + filepath, DRONE_TYPE);
            ...
            if( msg.isButtonDown() )
            {
                ...
                if (ACTIVE_MODE == SEQ_MODE) {
                  ...
                  if (msg.which == KEY_UP) {
                    ...
                    scale[Math.random2(0, scale.cap()-1)] => g.GRAIN_SCALE_DEG;
